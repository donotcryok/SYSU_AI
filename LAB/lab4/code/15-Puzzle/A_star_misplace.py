...
def a_star_search(initial_state):
    ...
    # 优先队列 - (f值, 移动步数, 状态, 父状态)
    open_set = [(get_misplaced_tiles(initial_state), 0, initial_state, None)]
    # 使用字典记录已访问状态的最佳路径信息
    closed_set = {}
    ...
    while open_set:
        ...
        for neighbor in get_neighbors(current_state):
            ...
            # 计算f值 = g值(移动步数) + h值(错位块数)
            f_value = new_moves + get_misplaced_tiles(neighbor)
...
def main():
    initial_state_2d = INITIAL_STATE_3  # 可以修改为任何一个初始状态
    ...
    if solution:
        ...
        print("\n移动数字序列:", " -> ".join(map(str, moved_numbers)))
    else:
        print("\n无法找到解决方案!")
