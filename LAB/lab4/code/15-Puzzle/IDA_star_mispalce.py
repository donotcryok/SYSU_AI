...
def get_misplaced_tiles(state):
    ...
    for i, num in enumerate(state):
        if num != 0 and num != GOAL_STATE[i]:  # 跳过空格，并检查是否在正确位置
            misplaced += 1
    return misplaced
...
def ida_star_search(initial_state):
    ...
    # 初始的深度限制为启发式函数值（错位方块数）
    bound = get_misplaced_tiles(initial_state)
...
def main():
    ...
    print("初始状态检查有解，继续求解...")
    print(f"初始错位方块数: {get_misplaced_tiles(initial_state)}")
